...
class DNN:
    ...
    def train_model(self, data, epoch=10, bs=256, verbose=0, tensor_board_name=None):
        ...
        if tensor_board_name is not None:
            ...
            history_training = self.model.fit(x=data.X_tr, y=data.y_tr, batch_size=bs, epochs=epoch, verbose=verbose, callbacks=tensorboard_callback)

        else:
            # the keras command to launch the training routine
            history_training = self.model.fit(x=data.X_tr, y=data.y_tr, batch_size=bs, epochs=epoch, verbose=verbose)
